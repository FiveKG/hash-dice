// @ts-check
const logger = require("../../common/logger.js");
const { Decimal } = require("decimal.js");
const { getUserBalance } = require("../balance");
const { WITHDRAW_ENABLE, REPEAT_CURRENCY, LOTTO_CURRENCY, GAME_CURRENCY, BASE_RATE } = require("../../common/constant/balanceConstants.js")
/**
 * 系统资产变动
 * @param { any } client 指定的数据库实例
 * @param { String } accountName 资产类型
 * @param { Decimal } changeAmount 变动的额度
 * @param { String } opType 操作类型
 * @param { String } remark 备注
 * @param { any } now 插入数据的时间
 */
async function personalAssetChange(client, accountName, changeAmount, opType, remark, now) {
    try {
        let rows = await getUserBalance(accountName);
        if (!rows) {
            throw Error("this account not exist");
        }
        let preAmount = rows.amount;
        let currentBalance = changeAmount.add(preAmount).toFixed(8);
        let updateAmountSql = `
            update balance set withdraw_enable = withdraw_enable + $1 where account_name = $5;
            update balance set repeat_currency = repeat_currency + $2  where account_name = $5;
            update balance set lotto_currency = lotto_currency + $3 where account_name = $5;
            update balance set game_currency = game_currency + $4 where account_name = $5;
            insert into balance_log(account_name, change_amount, current_balance, op_type, remark, create_time)
                values($5, $6, $7, $8, $9, $10);
        `
        logger.info("update personal amount");
        const opts = [ 
            changeAmount.mul(WITHDRAW_ENABLE / BASE_RATE).toFixed(8), 
            changeAmount.mul(REPEAT_CURRENCY / BASE_RATE).toFixed(8),
            changeAmount.mul(LOTTO_CURRENCY / BASE_RATE).toFixed(8),
            changeAmount.mul(GAME_CURRENCY / BASE_RATE).toFixed(8),
            accountName,
            changeAmount,
            currentBalance,
            opType,
            remark,
            now
        ]
        console.debug("the sql is %s, the opts is %O", updateAmountSql, opts);
        await client.query(updateAmountSql, opts);
        logger.info(`update '${ accountName }' amount ok`);
    } catch (err) {
        console.error("update %s amount error, the error stock is %O", accountName, err);
        throw err;
    }
}

module.exports = personalAssetChange;