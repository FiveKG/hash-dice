// @ts-check
const logger = require("../../common/logger.js").child({ [`@${ __filename }`]: "可售额度" });
    ...
    try {
        ...
        resData["data"] = {
            ...
            "detail": balanceLogInfo.filter(it  => {
                return (it.op_type === OPT_CONSTANTS.BUY || it.op_type === OPT_CONSTANTS.SELL) && it.extra.op_type === 'sell_amount'
            }).map(it => {
                return {
