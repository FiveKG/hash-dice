  ...
  <ons-page>
    <div class="wrap">
      ...
      <div class="pub-header">
        <div class="logo"><img src="@/assets/img/avatar.aa6fb79c.jpg" alt="" @click="$router.go(-2);"></div>
        <div class="title">{{$store.state.wallet.localFile.wallets[0].accountNames[0]}}</div>
        <div class="action">
          <div class="more" @click="actionSheetVisible = true"><img src="@/assets/img/assembly_ic_option@2x.png" alt=""></div>
          <div @click="$router.go(-2);" class="exit"><img src="@/assets/img/assembly_close_ic@2x.png" alt=""></div>
        </div>
      </div>
        ...
        <div class="container">
          <div class="recording">
            <transition-group name="scroll" >
              <div style="display:inline-block" class="row list-complete-item" v-for="item in BlockItems" :key='item.treasureKey'>
                  <div class="display_ib" style="width: 23%;height: 100%;color:#bcbcbc;font-size:0.25rem"><p class=" font_four p_A" style="line-height: .64rem;text-align: center; ">{{item.block_num}}</p> </div>
                  <div class="display_ib" style="width: 54%;height: 100%;color:#bcbcbc;font-size:0.25rem"><p class=" font_four p_A" style="line-height: .64rem;text-align: center; ">{{item.id}}</p></div>
                  <div class="display_ib" style="width: 23%;height: 100%; color:#bcbcbc;font-size:0.25rem"><p class=" font_four p_A" style="line-height: .64rem;text-align: center; ">{{item.timestamp}}</p></div>
              </div>
            </transition-group> 
          </div>
        </div>
...
export default {
  ...
  watch: {
    '$store.state.wallet.block': {
      handler(newVal, oldVal) {
        this.treasureKey+=1;
        this.BlockItems.unshift({timestamp:format(this.$store.state.wallet.block.timestamp, 'HH:mm:ss:S'),block_num:this.$store.state.wallet.block.block_num,
        id:'...'+this.$store.state.wallet.block.id.slice(45),treasureKey:this.treasureKey});
        this.BlockItems.splice(10);
      }
    },
  },
  ...
  created(){
    ...
    //获取全球彩奖池，倒计时，期数
    this.getOpen()
    console.log("$store.state.wallet.localFile.wallets:",this.$store.state.wallet.localFile.wallets);
  }
}
</script>
...
.rand-bet-dialog .container .rand-bet{
  width: 90%;height: 1.2rem;margin: auto;margin-top: 1rem;background-color: rgb(218, 178, 121);border-radius: 0.2rem;text-align: center;
