    ...
    <slot>
      ...
      <div class="page_content">
        ...
        <div class="item">
          ...
          <span class="item_value copy" :data-clipboard-text="query.publicKey" @click="copy('public')">{{query.shortKey}}</span>
        </div>
      ...
      <v-ons-dialog
        ...
        <div class="dialog_layout">
          ...
          <div class="btn bg_private copy" :data-clipboard-text="privateKey" @click="copy('private')">{{$t('me.copy_key')}}</div>
        </div>
      </v-ons-dialog>
      <v-ons-dialog
        modifier="width"
...
export default {
  ...
  created() {
    ...
    // getCoinRate({coin_id: 'POG', convert: this.$store.state.wallet.currency}).then(res => {
      // console.log(res)
      // if (res.code === 1) {
        //按40元算
        this.query.assets = Decimal.mul(this.query.balance, 40).toFixed(2)
      // }
    // })
  },
  watch: {
  ...
  methods: {
    deleteWallet() {
      ...
      const isDefault = wallets.find(ele => ele.privateKey === this.query.privateKey).isDefault
      const index = wallets.findIndex(ele => ele.privateKey === this.query.privateKey)
      wallets.splice(index, 1)
      
      if (isDefault && wallets.length) {
        wallets[0].isDefault = true
        if(wallets[0].chain == 'eth'){
          localFile.invitationAccount = wallets[0].name;
        }else {
