...
export default {
  ...
  created() {
    console.log(this.$store.state.wallet.selectToken, this.$route.query.content)
      ...
      if (privateKey) {
        ...
        try {
          ...
          if (!this.contract || this.contract === 'eosio.token') {
            await eos.transfer(this.from, this.to, `${Decimal(this.amount).toFixed(4)} ${this.token}`, this.memo, opts)
          } else {
            // 转代币
            const adm = await eos.contract(this.contract)
