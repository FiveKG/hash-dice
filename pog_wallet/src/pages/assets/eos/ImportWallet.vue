    ...
    <slot>
      ...
      <div class="layout">
        <v-ons-row class="chain_type">
          ...
          <span class="row_content">{{$t('assets.eos_chain')}}</span>
        </v-ons-row>
...
export default {
  ...
  data () {
    return {
      tabIndex: 0,
      privateKey: '', //5KVL23nv9LGjChq8zqHv86ETcZ2FV68DHrniSBdXxMwmTF5ZusW, 5JwXZZmNaYHmVznRezfPNGHS8hBdEAnKqAFz81Q2SYwnpYRr6tz
      pwd: '',
      pwd2: '',
      hint: '',
      showPwd: false,
        ...
        if (this.privateKey.length === 51 && ecc.isValidPrivate(this.privateKey)) {
          ...
          if (elem) {
            ...
            for (let item of wallets) {
              ...
              if (item.chain === 'eos' && item.accountNames[0] === account.account_names[0]) {
            ...
            //   this.$store.commit('wallet/setAssets', null)
            //   setTimeout(() => {
            //     this.$router.go(1-this.$route.matched.length)
            //   }, 20);
            // } else {
