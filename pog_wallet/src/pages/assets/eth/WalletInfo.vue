...
export default {
  ...
  created() {
    ...
    getCoinRate({coin_id: 'ETH', convert: this.$store.state.wallet.currency}).then(res => {
      // console.log(res)
      if (res.code === 1) {
  ...
  methods: {
    deleteWallet() {
      ...
      const isDefault = wallets.find(ele => ele.address === this.query.publicKey).isDefault
      const index = wallets.findIndex(ele => ele.address === this.query.publicKey)
