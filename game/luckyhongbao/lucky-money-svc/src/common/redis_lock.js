...
function acquireLock( lockName, timeout, retryDelay, onLockAcquired) {
	function retry() {
		setTimeout(function() {
	...
	redis.set(lockName, lockTimeoutValue, 'PX', timeout, 'NX', function(err, result) {
		if(err || result === null) return retry();
		onLockAcquired(lockTimeoutValue);
	});
}

module.exports = function( retryDelay) {
	// if(!(redis && redis.setnx)) {
	...
	function lock(lockName, timeout, taskToPerform) {
		...
		lockName = "lm:lock." + lockName;

		acquireLock( lockName, timeout, retryDelay, function(lockTimeoutValue) {
			taskToPerform(promisify(function(done) {
				done = done || function() {};

				if(lockTimeoutValue > Date.now()) {
					redis.del(lockName, done);
				} else {
					done();
				}
			}));
