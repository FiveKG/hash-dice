   ...
   class [[eosio::contract("eosio.token")]] token : public contract {
      public:
         ...
         token(name receiver, name code, datastream<const char*> ds):contract(receiver, code, ds) {
            configs conftable(receiver, receiver.value);
            if (conftable.begin() == conftable.end()) {
               vector<name> admin;
          ...
          *
          * @param admin 
          * @param open_time 交易开放时间
          * @param close_time 交易关闭时间
          */
         [[eosio::action]]
         void setconfig( const name& admin, const uint32_t open_time, const uint32_t close_time );
      ...
      private:
         // 交易时间 10 - 21
         const uint32_t HOURS_SECONDS = hours(1).to_seconds(); // 一小时包含的秒数
         const uint32_t TRX_OPENING_HOURS = HOURS_SECONDS * (10 - 8); // 交易开放时间
         const uint32_t TRX_CLOSING_HOURS = HOURS_SECONDS * (21 - 8); // 交易关闭时间
         const uint32_t ONE_DAY = days(1).to_seconds();  // 一天包含的秒数
         static constexpr name test_account{"yujinsheng11"};
         const symbol TBG_SYMBOL = symbol(symbol_code("TBG"), 4);
         static constexpr uint64_t LIMIT_AMOUNT = 210000000000;
         const asset RETIRE_LIMIT{ LIMIT_AMOUNT, TBG_SYMBOL };
         ...
         struct [[eosio::table]] admin_config {
            name creator;
            vector<name> admin_name;
            uint32_t trx_opening_hours;
            uint32_t trx_closing_hours;
         ...
         typedef eosio::multi_index< "config"_n, admin_config > configs;
